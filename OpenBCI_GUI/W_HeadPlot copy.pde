...
class HeadPlot {
    ...
    public void draw() {
     ...
     //   scene.stroke(125, 125, 125);
      ...
      //  ellipse(earR_x, earR_y, ear_width, ear_height); //little circle for the ear
        ...
        for (int Ielec=0; Ielec < electrode_xy.length; Ielec++) {
            ...
            sceneHead.fill(electrode_rgb[0][Ielec], electrode_rgb[1][Ielec], electrode_rgb[2][Ielec]);
            sceneHead.noStroke();
         
            
            sceneHead.translate(sceneHead.width/2, sceneHead.height/2, 0);
            sceneHead.translate(electrode_xy[Ielec][0], electrode_xy[Ielec][1], elec_relXYZ.getFloat(Ielec, 2));
            sceneHead.lights();
           
            sceneHead.sphere(elec_diam/3); //electrode circle
     ...
     // sceneHead.endDraw();
       ...
       sceneHead.pushMatrix();  
       //sceneHead.beginDraw();   
        sceneHead.fill(255, 255, 255, 255);  //fill in a white head
        sceneHead.noStroke();
       // translate(circ_x, circ_y , -200);
        sceneHead.lights();
        //* .86602

        sceneHead.perspective(fov, float(width) / float(height), cameraZ/100.0, cameraZ*100.0);
       ...
       // println(modelX(0, 0, 0));  
        //scene.translate(5,12, -15);
        sceneHead.translate(sceneHead.width/2, sceneHead.height/2, 0);
        sceneHead.translate(5,12, -15);
        sceneHead.rotateZ(PI);
        //rotateX(radians(30));
        sceneHead.rotateY(-PI/24);
        ////sphere(circ_diam/1.5); //big circle for the head
        sceneHead.shape(head);
